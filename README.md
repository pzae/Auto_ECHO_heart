...
*Данный проект разработан на основе медицинской платформы "Экосистема Альфа". Код и данные под NDA. За сотрудничеством прошу писать на почту auglazunov@mail.ru*
...
* **Погрешность измерений** должна составлять менее **15%** по сравнению с результатами, полученными специалистом по УЗИ. По предварительным результатам, это требование является достижимым.  
* **Время обработки** одного видео должно исчисляться минутами, а не десятками минут.  
* **Определяемые характеристики сердца по УЗИ:**

1. **Левый желудочек (ЛЖ):**
   - **КДР** — конечно-диастолический размер  
   - **КСР** — конечно-систолический размер  
   - **КДО** — конечно-диастолический объём  
   - **КСО** — конечно-систолический объём  
   - **ФВ (фракция выброса)** — рассчитывается по формуле  
     $$
...
2. **Правый желудочек (ПЖ):**
   ...
   - **ФИП (фракция изменения площади)** — рассчитывается по формуле  
    $$
     \text{ФИП} = \frac{S_\text{диастолы} - S_\text{систолы}}{S_\text{диастолы}} \times 100\%
     $$
2. **Соотношение КДР ЛЖ / КДР ПЖ**
...
На их основе далее рассчитывается **фракция выброса (ФВ)**.
...
1. **Сегментация сердечных структур на УЗИ-видео**  
   Чтобы вычислить характеристики сердца, сначала нужно точно выделить интересующие области — предсердия и желудочки — на кадрах УЗИ. Это задача **сегментации** в компьютерном зрении, где модель должна не просто находить объекты, в отличие от задачи детекции, а точно выделять их форму и контуры. Для её решения мы используем фреймворк `ultralytics`, реализующий архитектуру `YOLO` (You Only Look Once).
...
2. **Вычисление параметров сердца**  
   ...
   - производные параметры: **фракция выброса (ФВ)** и **фракция изменения площади (ФИП)**.  
...
Сначала кратко опишем данные, затем объясним используемые метрики, а потом расскажем о результатах обучения и получения параметров сердца.
...
Для каждого класса и кадра считаются:

$$
Precision = \frac{TP}{TP + FP}, \quad Recall = \frac{TP}{TP + FN}
$$​

где TP — корректно найденные объекты при IoU ≥ 0.75, FP — ложные срабатывания, FN — пропущенные объекты.  
Далее для каждого класса вычисляется среднее значение AP, а затем усредняется по всем классам.
...
- IoU начинает расти сразу с 0.7-0.8 - модель довольно быстро учится определять нужные сегменты, а вот map75 стартует значительно ниже, на некоторых валидационных изображениях наблюдаются ложно-положительные предсказания.
- mAP@75 обычно выходит к плато чуть раньше, чем IoU: модель довольно быстро учится определять сегмент на изображении и не давать ложных срабатываний, однако его точные контуры уточняются в процессе обучения.
- Ширина доверительного интервала по IoU (IQR) сужается по мере обучения — вариативность сегментации между объектами падает.
- Разрыв train–val небольшой и стабильный: пример 0.03–0.06 по обеим метрикам. Переобучение незначительно. При этом тренировочная выборка почти полностью выучена моделью - IoU почти 1.
...
Также по результатам предсказаний модели на нескольких объектах из валидационной выборки были собраны отдельные изображения. Для наглядной демонстрации процесса обучения было собрано видео для одного объекта: розовым показана разметка специалиста, а каждому сегменту сердца соответствует свой цвет.
...
│   ├── ИИ_ИЮ_25.txt                # Оценка параметров сердца по ИИ видео,
│   │                               # выполненная врачами
